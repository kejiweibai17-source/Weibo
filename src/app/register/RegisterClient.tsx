  ...
  async function handleRegister(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ email, username, password, birthday, ref }),
      });
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md text-center space-y-6">
          ...
          <p className="text-gray-600 leading-relaxed text-sm">
            我們已寄出一封{" "}
            <span className="font-semibold text-gray-900">信箱驗證信</span> 到：
            <br />
  ...
  return (
    <div className="min-h-screen flex bg-white">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-blue-600 text-white relative flex-col justify-between overflow-hidden">
        ...
        {/* 視覺圖與輪播文字 (加入 Framer Motion AnimatePresence) */}
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 relative">
        <div className="w-full max-w-md space-y-8">
          <div className="text-center space-y-2">
            <h2 className="text-3xl font-bold text-gray-900">建立會員帳號</h2>
            <p className="text-gray-500 text-sm">開始您的專屬健康生活體驗</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition"
                disabled={isAnyLoading}
                required
              />
            </div>

  ...
  return (
    <>
      <style>{`
        ...
        @media (max-width: 768px) { .top-left-logo { top: 1.5rem; left: 1.5rem; } .bottom-left-text { bottom: 1.5rem; left: 1.5rem; font-size: 0.65rem; } .bottom-right-scroll { display: none; } .side-indicators { right: 1.5rem; } }
      `}</style>
      ...
      <section className="hero-container" ref={containerRef}>
        <div className="images-wrapper">
          {slides.map((slide, idx) => (
            <div key={`img-${idx}`} className="slide-image" ref={(el) => (imagesRef.current[idx] = el)}>
              <img src={slide.image} alt="完美紳士 MATEBOX 情境圖" />
            </div>
          ))}
        </div>
        ...
        <div className="top-left-logo">
          ...
          <span className="logo-sub">SMASMALL IRON SERIES</span>
        </div>

        <div className="bottom-left-text">
          <span className="underline">PERFECT GENTLEMAN MATEBOX 3IN1</span>

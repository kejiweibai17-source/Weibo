 ...
 * 版本：1.5.3
 * - 背景文字、產品影片區塊可開關「前台顯示」
 * - 產品影片：YouTube 網址／ID 改為選填（僅產品主圖必填）
...
function smasmall_series_block_help_text(string $type): string
{
    switch ($type) {
        ...
        case 'product_video':
            return '對應詳細頁產品展示與影片彈窗。需產品主圖；YouTube 網址／ID 選填（留空則前台不顯示播放按鈕）。可關閉「前台顯示」隱藏整段，不必清空內容。';
        default:
            return '';
    }
}
...
function smasmall_series_render_block_body(string $type, array $block, int $index): void
{
    ...
    if ($type === 'product_video') {
        ...
        <p><label>影片網址或 YouTube ID（選填）</label><br />
            <input type="text" class="large-text" name="<?php echo esc_attr($prefix); ?>[videoUrl]" value="<?php echo esc_attr($block['videoUrl'] ?? ($block['youtubeId'] ?? '')); ?>" placeholder="https://www.youtube.com/watch?v=... 或影片 ID" /></p>

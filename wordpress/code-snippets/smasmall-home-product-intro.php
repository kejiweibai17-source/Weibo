...
function smasmall_home_product_intro_render_spec_row(array $spec, $index): void
{
    ...
    <tr class="shpi-spec-row">
        ...
        <td>
            <input type="text" class="regular-text shpi-spec-label" name="product_intro[specs][<?php echo esc_attr($index_key); ?>][label]"
                   value="<?php echo esc_attr($spec['label'] ?? ''); ?>" placeholder="例如：適用機型" />
            <input type="text" class="regular-text shpi-spec-value" name="product_intro[specs][<?php echo esc_attr($index_key); ?>][value]"
...
function smasmall_home_product_intro_render_page(): void
{
        ...
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="shpi-form">
            ...
            <table class="form-table" role="presentation">
                <tr>
                    ...
                    <td><label><input type="checkbox" name="product_intro[enabled]" value="1" <?php checked(!empty($data['enabled'])); ?> /> 在前台顯示</label></td>
                </tr>
                <tr>
                    <th scope="row">背景圖</th>
                    <td>
                        <div class="shpi-image-box">
                            <div class="shpi-thumb">
                                ...
                                <?php else : ?>
                                    <div class="shpi-thumb-placeholder" id="shpi-bg-placeholder"><span class="dashicons dashicons-format-image"></span></div>
                                    <img src="" alt="" id="shpi-bg-preview" style="display:none" />
                                <?php endif; ?>
                            </div>
                            <input type="hidden" name="product_intro[backgroundImage]" id="shpi-bg-url" value="<?php echo esc_attr($bg); ?>" />
                            <p>
                ...
                <tr>
                    ...
                    <td><input type="text" class="regular-text" id="shpi-subtitle" name="product_intro[subtitle]" value="<?php echo esc_attr($data['subtitle'] ?? ''); ?>" placeholder="上蓋特寫" /></td>
                </tr>
                <tr>
                    <th scope="row"><label for="shpi-title">標題</label></th>
                    <td><input type="text" class="large-text" id="shpi-title" name="product_intro[title]" value="<?php echo esc_attr($data['title'] ?? ''); ?>" /></td>
                </tr>
                <tr>
                    <th scope="row"><label for="shpi-description">描述</label></th>
                    <td><textarea class="large-text" rows="3" id="shpi-description" name="product_intro[description]"><?php echo esc_textarea($data['description'] ?? ''); ?></textarea></td>
                </tr>
            </table>
    ...
    <style>
        .shpi-admin { max-width: 860px; }
        .shpi-image-box .shpi-thumb img, .shpi-thumb-placeholder { max-width: 480px; width: 100%; border-radius: 8px; background: #1d2327; }
        .shpi-thumb-placeholder { min-height: 160px; display: flex; align-items: center; justify-content: center; }
        .shpi-spec-label, .shpi-spec-value { display: block; max-width: 420px; }
    </style>
    <?php
}
